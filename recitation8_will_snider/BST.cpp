...
// GOLD TODO
bool _isValid(Node* node, int min, int max) {
    if (!node) return 1;
    if (node->key < min || node->key > max) return 0;
    return (_isValid(node->left, min, node->key - 1) && _isValid(node->right, node->key + 1, max));
}

bool BST::isValidBST()
{
  //TODO Uncomment below and Complete this function, you can use any logic (add a helper function if you want)
  return _isValid(root, getMinValueNode(root)->key, getMaxValueNode(root)->key);
}
