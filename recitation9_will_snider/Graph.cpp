...
void Graph::printPath(int src, int dest)
{
  ...
  for(int i = 0; i < vertices.size(); i++)
  {
    ...
    //assumes findShortestPath has been called already
  for (vertex* curr = vDest; curr; curr = curr->pred) { //iterate until no further pred
      path.push_back(curr);
  }  
    ...
    while(!q.empty()){
        ...
        for( int x = 0; x < n->adj.size(); x++ )
        {
            ...
            if (!adjVert->visited) {
                ...
                adjVert->distance = n->distance + 1;
                adjVert->pred = n;
                if (adjVert->key == dest) return adjVert->distance;
                q.push(adjVert);
            }
        }
